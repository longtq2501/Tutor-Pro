...
export const DayDetailModal = ({ day, onClose, onAddSession, onDelete, onTogglePayment, onToggleComplete }: Props) => (
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4">
        {day.sessions.length === 0 ? (
          ...
          day.sessions.map((session) => (
            <div key={session.id} className={`group flex flex-col border rounded-xl p-4 transition-all ${session.completed ? 'bg-card border-border hover:shadow-md hover:border-primary/30' : 'bg-slate-50 dark:bg-muted/30 border-dashed border-slate-300 dark:border-slate-700'}`}>
